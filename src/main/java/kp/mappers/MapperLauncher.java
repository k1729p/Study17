...
public class MapperLauncher {
	...
	public static void launch() {
		...
		final Queue<String> labelsQueue = Stream.of("X", "Y", "Z").collect(Collectors.toCollection(LinkedList::new));
		...
		String message = String.format(
				"Source first:         id[%d], name[%s],     top.left[%s], listOne size[%d], mapOne key/value[%d/%d]",
		...
		message = String.format(
				"Mapped to second:     id[%d], name[%s], bottom.right[%s], listTwo size[%d], mapTwo key/value[%d/%d]",
		...
		message = String.format(
				"Mapped back to first: id[%d], name[%s],     top.left[%s], listOne size[%d], mapOne key/value[%d/%d]",
		...
		message = String.format("Result first.listOne%s", firstResult.listOne());
		logger.info(message);
