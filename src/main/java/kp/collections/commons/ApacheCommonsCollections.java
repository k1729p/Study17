...
/**
 * Researching
 * <a href="https://commons.apache.org/proper/commons-collections/">Apache
...
public class ApacheCommonsCollections {
	...
	public static void researchPatriciaTrie() {
		...
		final UnaryOperator<String> valueMapper = ignoredValue -> String.valueOf(atomic.incrementAndGet());
		...
		message = String.format("%s", prefixMap.keySet());
		logger.info(message);
		logger.info(Constants.SEPARATOR_LINE);

		final String firstKey = patriciaTrie.firstKey();
		message = String.format("First key[%s], value[%2s], its previous key[%4s], its next key[%4s]", firstKey,
				patriciaTrie.get(firstKey), patriciaTrie.previousKey(firstKey), patriciaTrie.nextKey(firstKey));
		logger.info(message);

		final String otherKey = "bbb";
		message = String.format("Other key[%s], value[%2s], its previous key[%4s], its next key[%4s]", otherKey,
		...
		message = String.format("Last  key[%s], value[%2s], its previous key[%4s], its next key[%4s]", lastKey,
